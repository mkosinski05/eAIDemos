...
int8_t print_result(Image* img)
{
    ...
    for (i = 0; i < det.size(); i++)
    {
        ...
        stream << "Class " << ":" << label_file_map[det[i].c].c_str() << " " <<  std::round(det[i].prob*100) << "%";
        str = stream.str();
    ...
    img->write_string_rgb(str, DRPAI_IN_WIDTH * RESIZE_SCALE + TEXT_WIDTH_OFFSET, LINE_HEIGHT+LINE_HEIGHT_OFFSET+1*LINE_HEIGHT, CHAR_SCALE_LARGE, WHITE_DATA);
