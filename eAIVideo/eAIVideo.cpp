...
/*
    ...
    gst-launch-1.0 filesrc location=./Road.mp4 ! qtdemux ! queue ! h264parse ! queue ! omxh264dec ! queue ! waylandsink
      ...
      char *argv[])
{
  ...
  GstElement *pipeline, *source, *demuxer, *queue1, *queue2, *queue3, 
  		*parser, *decoder, *down_scale, *sink;
  GstBus *bus;
  guint bus_watch_id;
  ...
  vspm  = gst_element_factory_make ("vspmfilter",       "vspm");
  sink	= gst_element_factory_make ("waylandsink", 		"video_sink");

  if (!pipeline || !source || !demuxer || !queue1 || !queue2 || !queue3 || !parser || !decoder || !down_scale || !sink) {
  ...
  /* */
  g_object_set (G_OBJECT (down_scale), "dmabuf-use", "true, NULL);
  /* */
  g_object_set (G_OBJECT (down_scale), "location", argv[1], NULL);
  /* */
  g_object_set (G_OBJECT (down_scale), "location", argv[1], NULL);
